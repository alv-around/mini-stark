...
use mini_starks::field::{BabyBear, BabyBearFp};
use mini_starks::starks::{Stark, StarkConfig};
use sha2::Sha256;

const ONE: BabyBearFp = BabyBearFp::ONE;
const ZERO: BabyBearFp = BabyBearFp::ZERO;
...
impl Provable<Witness, BabyBearFp> for FibonacciClaim {
    fn trace(&self, witness: &Witness) -> TraceTable<BabyBearFp> {
...
fn test_stark_prover() {
    ...
    let config =
        StarkConfig::<Sha256, BabyBear>::new(20, blowup_factor, trace.step_number(), columns);
