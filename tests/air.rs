...
fn test_fibonacci_air_constrains() {
    ...
    let boundary = constrains.get_constrain(1).clone() * z;
    assert_eq!(boundary.evaluate(&omega_4), ZERO);

    let carry_over_constrain = constrains.get_constrain(2).mul_by_vanishing_poly(domain);
    let sum_constrain = constrains.get_constrain(3).mul_by_vanishing_poly(domain);
